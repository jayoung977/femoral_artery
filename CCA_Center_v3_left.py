...
##Root_Save_Path###
sub_folder_path = [ os.path.join(Root_Save_Path,x) for x in folder]
sub_folder_p = [os.listdir(x) for x in sub_folder_path ]
sub_folder_p #0:both, 1:left, 2: right
...
all_data_copy = both_left_path+both_right_path+left_path+right_path
#%%
# len('C:/Users/user/Desktop/femoral artery/2022.01.21_KH//')
#%%
# len(all_data)
# len(all_data_copy)
...
# all_data_copy[0][66:]
# #%%
all_data_copy_2 =  [x[66:] for x in all_data_copy ]
#%%
need_data = [all_data[x] for x in range(len(all_data)) if all_data[x][51:] not in all_data_copy_2]
# need_data
# len(need_data)
print(len(all_data),len(all_data_copy),'need_data:',len(need_data))
print(len(all_data_copy)+len(need_data))
#%%
# need_data[0]
#%%
