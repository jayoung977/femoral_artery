...
def set_parameter_requires_grad_unfreeze(model, where_unfreeze):
    for name, param in model.named_parameters():
        if(where_unfreeze == 1):
            ...
            if (('denseblock4' in name) or ('denseblock3' in name) or ('denseblock2' in name)):
                param.requires_grad = True
        elif(where_unfreeze == 4):
            if (('denseblock4' in name) or ('denseblock3' in name) or ('denseblock2' in name) or ('denseblock1' in name)):
                param.requires_grad = True        
...
def main(args):
    ...
    for epoch in range(start_epoch, args.Epoch):
        ...
        if(test_acc_mean > best_accuracy):
            ...
            } , os.path.join(Model_Save_Path + "Best_NOT_ACC_%.5f at Epoch_%d_Model.tar" % (test_acc_mean, epoch)))
        ...
        if (Break_Check == 0 and Break_Flag == 5):
            Break_Check = 1
            set_parameter_requires_grad_unfreeze(model, 1)
            Break_Flag = 1
            print("##### Breack_Check 1 #####")

        if (Break_Check == 1 and Break_Flag == 5):
            Break_Check= 2
            set_parameter_requires_grad_unfreeze(model, 2)
            Break_Flag = 1
            print("##### Breack_Check 2 #####")

        if (Break_Check == 2 and Break_Flag == 5):
            Break_Check= 3
            set_parameter_requires_grad_unfreeze(model, 3)
            Break_Flag = 1
            print("##### Breack_Check 3 #####")

        if (Break_Check == 3 and Break_Flag == 10):
            Break_Check= 4
            set_parameter_requires_grad_unfreeze(model, 4)
            Break_Flag = 1
            print("##### Breack_Check 4 #####")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_num', type=str, default="5", help="GPU number configuration")
    ...
    parser.add_argument('--Save_Path', type=str, default="/mnt/NAS01/Jayoung/femoral_artery/data2/View_Classification/Model_Save/densenet_pretrain/", help="GPU number configuration")
