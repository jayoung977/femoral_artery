...
torch.cuda.manual_seed_all(10) # if use multi-GPU
torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False
np.random.seed(10)
random.seed(10)
...
class Dataset():
    def __init__(self, path_list, transform, transform2, istrain):
        ...
        if istrain == True:    #train
            patient = target_inhouse.train_patient
            target_class = np.array(target_inhouse.target_class)
            print('----train----')
            print(patient)
        else:                  #test
            patient = target_inhouse.test_patient
            target_class = np.array(target_inhouse.target_class)
            print('----test----')
            print(patient)
        ...
        for i in range(len(path_list)):
            ...
            # depth_info = int(path_list[i].split('/')[-1][-6:-4])
            depth_info = path_list[i].split('/')[-1].split('Scale')[-1].split('.png')[0]
            depth_info = (float(depth_info) - 20.0) / (70.0 - 20.0)
            ...
            if(str(before_class_num) in target_class):                
                ...
                if patient_num in patient:
                    ...
                    self.Img_depth_list.append(torch.tensor(depth_info))
    ...
    def __getitem__(self, index):
        ...
        if (self.istrain == True):
            ...
            Is_Resize_and_Resize_with_Padding = -1
            Is_ShearX, Is_ShearY = -1, -1
            Is_TranslateXabs, Is_TranslateYabs = -1, -1
            Is_Rotate = -1
