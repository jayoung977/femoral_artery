...
def add_polygon_light(image, intensity = 0.5, blur_width = 6, n_sides = 3):
	image = np.array(image)
	inverted_colors = const.WHITE - image
	inverted_shadow = add_polygon_shadow(inverted_colors, intensity, blur_width, n_sides)
	return const.WHITE - inverted_shadow

# tip: just stick with triangles, other polygons have incoherent shades
def add_polygon_shadow(image, intensity = 0.5, blur_width = 6, n_sides = 3):
	image = np.array(image)
