...
def computeHDR(images, log_exposure_times, smoothing_lambda=100., gamma=0.6):
    ...
    for channel in range(num_channels):
        ...
        hdr_image[:, :, channel] = cv2.normalize(img_rad_map, None, 0, 255, cv2.NORM_MINMAX)
    ...
    print(np.min(image_tuned), np.max(image_tuned))
    
    # Output image
    # output = cv2.normalize(image_tuned, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    output = cv2.normalize(image_tuned, None, 0, 255, cv2.NORM_MINMAX)
