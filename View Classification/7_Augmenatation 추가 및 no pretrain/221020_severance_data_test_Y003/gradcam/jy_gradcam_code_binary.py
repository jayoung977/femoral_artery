...
def main(args):
    ...
    #color 
    root_path = "/mnt/NAS01/Jayoung/femoral_artery/data2/View_Classification/Vessel_Color/"
    ...
    last_model_root_path = "/mnt/NAS01/Jayoung/femoral_artery/data2/View_Classification/Model_Save/"+args.Img_Save_Path.split('/')[-2].replace('_binary','') +'/' + target.target_name +'/'
    ...
    for Test_Iter, (V_Images, v_Labels, file_path) in enumerate(test_loader):
        ...
        grad_cam_map = grad_cam_map.squeeze().cpu()
        #threshold : 0.3
        grad_cam_map = torch.where(grad_cam_map >= 0.3, 1, 0)
   
        # grad_cam_map.squeeze() : (224, 224)
        # grad_heatmap = cv2.applyColorMap(np.uint8(255 * grad_cam_map), cv2.COLORMAP_JET) # (224, 224, 3), numpy 
        ...
        grad_heatmap = np.uint8(255 * grad_cam_map)
        grad_heatmap = cv2.merge((grad_heatmap,grad_heatmap,grad_heatmap))
        grad_heatmap = torch.from_numpy(grad_heatmap).permute(2, 0, 1).float().div(255) # (3, 224, 224)
        b, g, r = grad_heatmap.split(1)
        rad_heatmap = torch.cat([r, g, b]) # (3, 244, 244), opencv's default format is BGR, so we need to change it as RGB format.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Img_Save_Path', type=str, default="/mnt/NAS01/Jayoung/femoral_artery/data2/View_Classification/Gradcam_Save/color_efficientnet_pretrain_binary/", help="GPU number configuration")
